...
export async function POST(request: Request) {
  ...
  if (count > 0) return NextResponse.json({ error: 'Setup already completed' }, { status: 403 })
  ...
  const admin = await prisma.user.create({ data: { name, phone, role: 'ADMIN', faceDescriptor: descriptor } })
